 ...
 */
function buildSchemaForHero(eventTypeName, versionNumber = 1, sourceName, jsonSchema) {
  return {
  ...
  return {
    ...
    objects: {
      postProcessFnc: (schema, obj, defaultFunc) => (
        {...defaultFunc(schema, obj), required: Object.getOwnPropertyNames(obj)}),
...
function addSchemaData(options) {
  options.objects.postProcessFnc = (schema, obj, defaultFunc) => (
     {...defaultFnc(schema, obj), required: Object.getOwnPropertyNames(obj), ...schemaConfig})
};
  ...
  const newArr = arr.map(prop => {
    ...
    switch (prop.typeOfProp) {
      ...
      case 'object':
        ...
        break;
      case 'boolean':
        prop.valOfProp.toLowerCase() === 'true' ? result[prop.property] = true : result[prop.property] = false;
        break;
      }
    return result
  });
