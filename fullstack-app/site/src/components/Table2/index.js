...
class RenderTable extends Component {
  ...
  data = () => {
    const initialDataSet = this.props.registeredEvents.map((data, index) => ({
      ...data,
      typeDefinition: JSON.stringify(data.typeDefinition),
      key: index,
    }));

    if (
      this.props.selectedSources.length === 0 ||
      this.props.selectedSources[0] === "All Sources"
    ) {
      return initialDataSet;
    } else {
      const filteredDataSet = initialDataSet.filter((obj) =>
        this.props.selectedSources.includes(obj.source)
      );
          ...
          <Content>
            <div>
              <Table
                ...
                bordered
                title={() => (
                  <h3 style={{ color: "#7b94b7" }}>
                    View log of events that have occured for a given source
                  </h3>
                )}
                footer={() => (
                  <h5 style={{ color: "#7b94b7" }}>Insight RX Hero</h5>
                )}
              />
            </div>
          </Content>
