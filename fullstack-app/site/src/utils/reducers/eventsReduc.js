  ...
  state = {
    ...
    switch (action.type) {
          case actions.SET_SELECTEDEVENTTYPE:
            ...
            return ({...state, query: {...state.query, "source-name": action.payload},
            reportQuery: {...state.reportQuery, "source-name": action.payload}})

            case actions.SET_REGISTEREDEVENTS:
                return ({...state, registeredEvents: action.payload})
                
            case actions.SET_EVENTTYPE:
                return ({...state, query: {...state.query, "eventtype-name": action.payload}})

            case actions.SET_EVENTSREPORT:
              return ({...state, eventsReport: action.payload})

            case actions.SET_VERSIONNUMBER:
                return ({...state, query: {...state.query, "version-number": action.payload}})
      
            case actions.SET_STARTDATE:
              return ( {...state, query: {...state.query,"start-date": action.payload}, reportQuery: {...state.reportQuery, "start-date": action.payload}})
              
            case actions.SET_ENDDATE:
              return ( {...state, query: {...state.query,"end-date": action.payload}, reportQuery: {...state.reportQuery, "end-date": action.payload}})
